...
## 1. if / else if / else: 조건을 `boolean`으로 판단해요.
...
`break`가 없으면 다음 `case`의 코드까지 이어서 실행될 수 있어요.
...
## 5. `continue`는 이번 반복을 건너뛰고, `break`는 반복문을 종료해요.
...
`const`는 각 반복마다 새로 만들어지는 변수이므로 사용할 수 있어요.
